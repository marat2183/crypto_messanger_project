...
def index(request):
    ...
    #         fail_silently=False
    #     )
    return render(request, template_name='base.html', context={"title": 'Главная'})
        ...
        if request.is_ajax() and request.method == 'POST':
            ...
            if 'login' in request.POST.keys() and 'password' in request.POST.keys() and 'second_password' in request.POST.keys():
                ...
                if validate_data(user_login, user_password,
                                 user_second_password) and user_password == user_second_password:
                    try:
                        user_data = Users(login=user_login, password=user_password)
                        user_data.save()
...
def login(request):
    if 'user_id' in request.session.keys():
        ...
        if request.method == 'POST' and 'login' in request.POST.keys() and 'password' in request.POST.keys():
            ...
            key_vaild = check_crypto(word, user_address)
            if res and key_vaild:
                ...
                data = {'message': 'bad'}
                return HttpResponse(json.dumps(data), content_type='application/json')
        elif request.method == "POST" and 'login' not in request.POST.keys():
            generate_secret()
